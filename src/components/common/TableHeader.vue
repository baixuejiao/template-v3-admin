...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      const title = ref<String>('基本操作')
      let searchContentHeight = ref<String>('175px')
      ...
      onMounted( () => {
        if(props.count > 4 && props.count <= 6) {
          searchContentHeight.value = '240px'
        } else if(props.count > 6) {
          searchContentHeight.value = '300px'
        }
        showFilter.value = props.showFilter
      })
      return {
        showSearchContent,
        target,
